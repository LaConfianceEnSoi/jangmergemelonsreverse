...
저는 박장현 님의 한숨과 미아를 정말 좋아하고 특히 무쌍마초 팀에서 보여주신 리더십에 깊이 감사하며 예능에서 보여주시는 애드립에도 늘 감탄하고 있는 도토리입니다.

우리 다람지눅 손진욱에게 주신 관심과 지지에 감사하는 마음으로 장드컵 특별 선물을 만들어보았습니다.

짤드컵은 '움짤'로 하는 거라서 우승작을 웹사이트 대문 등에 활용하는 등 특별한 기념을 하기 어려울 수 있지만, 마침 수박게임에 사용되는 이미지가 총 16점이라는 점을 이용하여 장드컵 16강 짤에서 최대한 예쁜 장면을 포착한 16컷으로 수박 합치기 게임을 만들어보았습니다.

8강전부터 계속 모니터링하여 TOP 8 이미지가 더 크게 뜨고, 게임 플레이에 주로 활용되는 중간 크기 수박은 '1,2위 결승 진출 이미지 두 장'이 되도록 하였습니다.

왜 결승작을 가장 크게 하지 않고 중간 크기로 하였냐고요?

이번에 제가 드리는 특별한 수박게임은 '리버스 모드'이기 때문입니다.

다른 수박게임은 작은 과일 둘을 합치면 더 큰 과일이 되지만 리버스 모드는 큰 과일을 합치면 한 단계 더 작은 과일이 됩니다.
게임을 시작할 때 가장 큰 11번째 과일부터 시작하고 계속 합쳐나가서 마지막에 첫번째 과일을 만나는 거죠.

11번째, 10번째 과일은 현실적으로 너무 커서 두 개도 쌓기 힘들기 때문에 게임에 반복해서 나타나면 게임이 끝나버립니다. 

그래서 3,4위 짤을 11번째, 10번째에 배치하고 결승 짤은 9번째, 8번째 과일로 배치하여 1,2위를 차지한 장현님 얼굴을 반복해서 볼 수 있도록 하였습니다.

리버스 모드는 과일이 점점 작아지기 때문에 높이 쌓이지 않아서 내가 잠들거나 스마트폰 웹브라우저가 다운되지 않는 한 며칠이고 할 수 있다는 독특한 재미가 있습니다.

그럼 즐겜해주시고요...

참, 이 깃허브 소개 화면까지 찾아와주실 정도로 웹프로그래밍을 잘 다루시는 고수 조림님이시라면 이 화면에서 파일을 모두 다운받거나 fork하셔서 조림님 깃허브 계정에 소장하시는 것도 가능하다는 것도 아시겠죠? 그럼 즐겜하세요 :)
